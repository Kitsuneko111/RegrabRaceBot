...
class GUI:
    def __init__(self):
        ...
        app = self.app
        app.setIcon("kisspng-stopwatch-timer-clip-art-time-5ac30c04a83206.8478606515227320366889.jpg")
        app.setSticky("news")
    ...
    def mainclock(self):
        ...
        if teamLapped:
            ...
            if "b" in teamLapped:
                self.bgates += 1
                self.bgates %= 6
                #gates: str = self.app.getLabelTooltip("BLap"+str(self.blaps))
                #gates: str = gates[7:]
...
class API:
    def __init__(self):
        ...
        self.IP = config.read().splitlines()[0]
        config.close()
        self.orangeGates = [((15, 11), (2, -2), 0),
                            ((1, -1), (1, -1), -35),
                            ((-11, -15), (2, -2), 0),
                            ((1, -1), (1, -1), 35)]
        self.blueGates = [((-11, -15), (2, -2), 0),
                          ((1, -1), (1, -1), 35),
                          ((15, 11), (2, -2), 0),
                          ((1, -1), (1, -1), -35)]
    ...
    def checkRay(self, lastPos, currentPos, gate):
        ...
        intersection = self.LinePlaneCollision(planeNormal, planePoint, rayDirection, rayPoint)
        if intersection is not None:
            gate_1 = [gate[0][0], gate[1][0], gate[2]]
            gate_2 = [gate[0][1], gate[1][1], gate[2]]
            if gate_2[0] < intersection[0] < gate_1[0] and\
               gate_2[1] < intersection[1] < gate_1[1] and\
               gate_2[2] == intersection[2]:
                return True
        else:
            return False

    def LinePlaneCollision(self, planeNormal, planePoint, rayDirection, rayPoint, constant=1e-6):
        """
        code inspired by
        ...
        :return:
        """
        normalDotProduct = planeNormal.dot(rayDirection)
        if abs(normalDotProduct) < constant:
            return

        pointDelta = rayPoint - planePoint
        dotProdRatio = -planeNormal.dot(pointDelta) / normalDotProduct
        pointOfIntersection = pointDelta + dotProdRatio * rayDirection + planePoint
    ...
    def checkGates(self, res=None):
        ...
        orangePlayers = res["teams"][1]["players"]
        ...
        try:
            if self.checkRay(self.bluePositions[-1], bluePosAverage, self.blueGates[self.counterBlue]):
                ...
                print("blue passed through gate"+str(self.counterBlue))
        ...
        try:
            if self.checkRay(self.orangePositions[-1], orangePosAverage, self.orangeGates[self.counterOrange]):
                ...
                returnStr += "o"
                print("orange passed 4through gate"+str(self.counterOrange))
